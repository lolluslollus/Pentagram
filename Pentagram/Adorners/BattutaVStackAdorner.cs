...
namespace Pentagram.Adorners
{
	...
	{
		internal static readonly SolidColorBrush LINE_BRUSH = new SolidColorBrush(Colors.Black);
		internal static readonly SolidColorBrush _lineStrokeRed = new SolidColorBrush(Colors.Red);
		internal static readonly SolidColorBrush _lineStrokeBlue = new SolidColorBrush(Colors.Blue);
		internal static readonly double LINE_STROKE_THICKNESS = 1.0;
		...
		protected override void Draw()
		{
			...
			Task upd = RunInUiThreadAsync(() =>
			{
				...
				foreach (var battutaY in battutaYs)
				{
					...
					{
						var line = new Line() { X1 = 0.0, X2 = width, Y1 = lineY + battutaY, Y2 = lineY + battutaY, Stroke = LINE_BRUSH, StrokeThickness = LINE_STROKE_THICKNESS };
						_layoutRoot.Children.Add(line);
					}
					var vBarLeft = new Line() { X1 = 0.0, X2 = 0.0, Y1 = lineYs.First() + battutaY, Y2 = lineYs.Last() + battutaY, Stroke = LINE_BRUSH, StrokeThickness = LINE_STROKE_THICKNESS };
					_layoutRoot.Children.Add(vBarLeft);
					var vBarRight = new Line() { X1 = width, X2 = width, Y1 = lineYs.First() + battutaY, Y2 = lineYs.Last() + battutaY, Stroke = LINE_BRUSH, StrokeThickness = LINE_STROKE_THICKNESS };
		...
		internal static List<double> GetLineYs()
		{
